...
// ---------------- DOM 유틸 ----------------
function displaySubtitle(text, opts) {
    const player = document.querySelector(".html5-video-player");
    if (!player)
        return;
    let subtitleDiv = document.getElementById("my-extension-subtitle");
    if (!subtitleDiv) {
...
// ---------------- 드래그 & 크기조절 ----------------
function makeSubtitleDraggableAndResizable(subtitleDiv) {
...
function injectButton() {
  ...
  `;
    // 슬라이더 이벤트 적용
    const fontSizeSlider = popup.querySelector("#subtitle-font-size");
    ...
    const subtitleToggle = popup.querySelector("#subtitle-toggle");
    const subtitleLang = popup.querySelector("#subtitle-lang");
    ...
    playerControls.querySelectorAll(".ytp-button").forEach(b => {
        if (b.id === "my-extension-btn")
            return;
    ...
    `;
        document.head.appendChild(style);
        // 빨간색 채움 업데이트
        const slider = document.getElementById("subtitle-font-size");
        if (slider) {
